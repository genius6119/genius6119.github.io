...
配置nginx的代理服务器挂了，而两台应用服务器正常的话，整个应用也就挂了。这明显不符合实际的生产场景啊，当然也有解决方法：DNS解析到
多台代理服务器的ip，每台代理服务器nginx配置相同的内容，这样就完成了代理服务器的轮询和应用服务器的负载均衡，实现了一个高可用性的系统架构（这里先不考虑多数据库），当然实际架构肯定比这里写的复杂得多，希望我在以后的工作中可以遇到。
...
　　　　　　　　我昨天把.201服务器的项目关了忘记重新打开了，打开之后再发10次请求：
.80服务器日志：
```
11
13
